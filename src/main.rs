...
use ai_utils::{split_message, RequestMessage};
...
impl Bot {
    ...
    async fn process_ai_request(
        ...
        let provider_name = self.ai_provider.name();
        info!("Requesting response from {}", provider_name);

        let ai_response = match &self.ai_provider {
            AiProvider::Claude { token } => {
                get_claude_response(request_body.clone(), token, &self.client)
                    .await
                    .map_err(|e| e.to_string())
            }
            AiProvider::ChatGpt { token } => {
                get_gpt_response(request_body.clone(), token, &self.client)
                    .await
                    .map_err(|e| e.to_string())
            }
        };

        let generated_message = match ai_response {
            Ok(text) => text,
            Err(e) => {
                error!("Error {} response: {}", provider_name, e);
                // エラーの詳細をユーザーに通知
                let error_msg = format!("{} APIエラーが発生しました: {}", provider_name, e);
        ...
        let split_messages = split_message(&generated_message, DISCORD_MAX_LENGTH - 50); // メンションなどの余裕を持たせる

        if split_messages.is_empty() {
            info!("No content generated by {}", provider_name);
            return;
        }
...
impl EventHandler for Bot {
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        if is_inclued_bot_mention(&ctx, &msg) {
            self.process_ai_request(&ctx, &msg, None, None).await;
        }
        // 特定のサーバーの特定のフォーラムチャンネルでの処理（新機能）
            ...
            self.process_ai_request(&ctx, &msg, title.as_deref(), description.as_deref())
                .await;
        }
...
async fn main() -> anyhow::Result<()> {
    ...
    let provider_key = env::var("AI_PROVIDER").unwrap_or_else(|_| "claude".to_string());
    info!("AI provider preference: {}", provider_key);

    let ai_provider = match provider_key.to_lowercase().as_str() {
        "claude" => {
            let token = match env::var("CLAUDE_TOKEN") {
                Ok(token) => {
        ...
        "chatgpt" | "openai" => {
            let (token, source) = match env::var("OPENAI_API_KEY") {
                Ok(token) => (token, "OPENAI_API_KEY"),
                Err(openai_err) => match env::var("CHATGPT_TOKEN") {
                    Ok(token) => (token, "CHATGPT_TOKEN"),
                    Err(chatgpt_err) => {
                        error!(
                            "Neither OPENAI_API_KEY nor CHATGPT_TOKEN were found ({} / {})",
                            openai_err, chatgpt_err
                        );
                        return Err(anyhow::anyhow!("OPENAI_API_KEY or CHATGPT_TOKEN environment variable is required when AI_PROVIDER=chatgpt"));
                    }
                },
            };

            if token.is_empty() {
                error!("{} is empty!", source);
                return Err(anyhow::anyhow!("{} is empty", source));
            }

            info!("{} found (length: {})", source, token.len());
            AiProvider::ChatGpt { token }
        }
        other => {
            error!("Unsupported AI_PROVIDER value: {}", other);
            return Err(anyhow::anyhow!("Unsupported AI_PROVIDER value: {}", other));
        }
    };

    info!("AI provider resolved: {}", ai_provider.name());
    ...
    // クローンを作成して所有権の問題を回避
    let ai_provider_for_framework = ai_provider.clone();
    let ai_provider_for_bot = ai_provider.clone();
    ...
    let framework = poise::Framework::builder()
        ...
        .setup(move |ctx, ready, framework| {
            ...
            Box::pin(async move {
                ...
                Ok(Data {
                    ai_provider: ai_provider_for_framework,
    ...
    // Botの設定を作成
    let bot_handler = Bot {
        ai_provider: ai_provider_for_bot,
        client: reqwest::Client::new(),
        target_server_ids,
